...
def extract_roi(image, x0, y0, x1, y1): #coordinates of the first and second click
    row0, row1 = min(y0, y1), max(y0, y1) #normalize to be between y coordinates of clicks
    col0, col1 = min(x0, x1), max(x0, x1) #normalize to be between x coordinates of clicks
